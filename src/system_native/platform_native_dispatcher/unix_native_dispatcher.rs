...
use crate::system_native::platform_native_dispatcher::unix_helpers::{
...
fn get_cwd(stack_frames: &mut StackFrames) -> crate::error::Result<i32> {
...
pub fn mkdir0_wrp(args: &[i32], stack_frames: &mut StackFrames) -> crate::error::Result<Vec<i32>> {
...
fn mkdir0(path_ptr: i64, mode: i32, stack_frames: &mut StackFrames) -> crate::error::Result<()> {
...
fn unlink0(path_ptr: i64, stack_frames: &mut StackFrames) -> crate::error::Result<()> {
    let path = cstring_from_i64(path_ptr)?;
...
pub fn rmdir0_wrp(args: &[i32], stack_frames: &mut StackFrames) -> crate::error::Result<Vec<i32>> {
    let path_ptr = i32toi64(args[1], args[0]);
...
fn rmdir0(path_ptr: i64, stack_frames: &mut StackFrames) -> crate::error::Result<()> {
    let path = cstring_from_i64(path_ptr)?;
...
pub fn lstat0_wrp(args: &[i32], stack_frames: &mut StackFrames) -> crate::error::Result<Vec<i32>> {
    let path_ptr = i32toi64(args[1], args[0]);
    let attr_ref = args[2];
...
fn cstring_from_i64(ptr_value: i64) -> crate::error::Result<CString> {
...
fn copy_stat_attributes(attr_ref: i32, stat: FileStat) -> crate::error::Result<()> {
