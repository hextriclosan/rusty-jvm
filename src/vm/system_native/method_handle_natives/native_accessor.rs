...
pub fn native_accessor_newinstance0(constructor_ref: i32, args_ref: i32) -> Result<i32> {
    let (clazz_ref, slot, entire_array_args, parameter_types) = with_heap_read_lock(|heap| {
        let method_name = heap.get_instance_name(constructor_ref)?;
        let clazz_ref =
            heap.get_object_field_value(constructor_ref, method_name.as_str(), "clazz")?[0];
        let slot = heap.get_object_field_value(constructor_ref, method_name.as_str(), "slot")?[0];
        let parameter_types_ref =
            heap.get_object_field_value(constructor_ref, method_name.as_str(), "parameterTypes")?
                [0];
